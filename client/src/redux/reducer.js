  ...
  switch(action.type){
    ...
    case FILTER_BY: {
      ...
      if(action.payload === 'DB'){
        return{...state, filtered: state.gamesBackUp.filter((g) => (typeof game.id) === 'string')}
      }
      if(action.payload === 'API'){
        return{...state, filtered: state.gamesBackUp.filter((g) => (typeof game.id) === 'number')}
      }
      else{
        return{...state, filtered: state.gamesBackUp.filter((g) => {
          return games.genres.find((ge) => {
